...
void EditorUI::DrawViewport(f32 dt) {
    ...
    if (cam) {
        // Collect input state for ViewportInputManager
        ViewportInputState inp;
        inp.mousePos       = Vec2(mousePos.x, mousePos.y);
        inp.mouseDelta     = Vec2(ImGui::GetIO().MouseDelta.x, ImGui::GetIO().MouseDelta.y);
        inp.scrollDelta    = ImGui::GetIO().MouseWheel;
        inp.lmbDown        = ImGui::IsMouseDown(ImGuiMouseButton_Left);
        inp.lmbClicked     = ImGui::IsMouseClicked(ImGuiMouseButton_Left);
        inp.lmbReleased    = ImGui::IsMouseReleased(ImGuiMouseButton_Left);
        inp.mmbDown        = ImGui::IsMouseDown(ImGuiMouseButton_Middle);
        inp.mmbClicked     = ImGui::IsMouseClicked(ImGuiMouseButton_Middle);
        inp.rmbDown        = ImGui::IsMouseDown(ImGuiMouseButton_Right);
        inp.rmbClicked     = ImGui::IsMouseClicked(ImGuiMouseButton_Right);
        inp.rmbReleased    = ImGui::IsMouseReleased(ImGuiMouseButton_Right);
        inp.shiftHeld      = ImGui::GetIO().KeyShift;
        inp.ctrlHeld       = ImGui::GetIO().KeyCtrl;
        inp.altHeld        = ImGui::GetIO().KeyAlt;
        inp.keyW           = ImGui::IsKeyDown(ImGuiKey_W);
        inp.keyA           = ImGui::IsKeyDown(ImGuiKey_A);
        inp.keyS           = ImGui::IsKeyDown(ImGuiKey_S);
        inp.keyD           = ImGui::IsKeyDown(ImGuiKey_D);
        inp.keyQ           = ImGui::IsKeyDown(ImGuiKey_Q);
        inp.keyE           = ImGui::IsKeyDown(ImGuiKey_E);
        inp.keyF           = ImGui::IsKeyPressed(ImGuiKey_F);
        inp.keyDelete      = ImGui::IsKeyPressed(ImGuiKey_Delete);
        inp.numpad0        = ImGui::IsKeyPressed(ImGuiKey_Keypad0);
        inp.numpad1        = ImGui::IsKeyPressed(ImGuiKey_Keypad1);
        inp.numpad3        = ImGui::IsKeyPressed(ImGuiKey_Keypad3);
        inp.numpad5        = ImGui::IsKeyPressed(ImGuiKey_Keypad5);
        inp.numpad7        = ImGui::IsKeyPressed(ImGuiKey_Keypad7);
        inp.viewportHovered = vpHovered;

        m_ViewInput.Update(inp);
        ViewportAction action = m_ViewInput.GetAction();
        Vec2 delta = m_ViewInput.GetMouseDelta();

        // Dispatch action to EditorCamera
        switch (action) {
            case ViewportAction::Orbit:
                m_EditorCam.Orbit(delta.x, delta.y);
                break;
            case ViewportAction::Pan:
                m_EditorCam.Pan(delta.x, delta.y);
                break;
            case ViewportAction::Zoom:
                m_EditorCam.Zoom(m_ViewInput.GetScrollDelta());
                break;
            case ViewportAction::FlyBegin:
                m_EditorCam.BeginFly();
                break;
            case ViewportAction::FlyUpdate:
                m_EditorCam.FlyUpdate(delta.x, delta.y,
                    m_ViewInput.GetFlyForward(), m_ViewInput.GetFlyRight(), m_ViewInput.GetFlyUp(),
                    dt, m_ViewInput.IsSprinting());
                break;
            case ViewportAction::FlyEnd:
                m_EditorCam.EndFly();
                break;
            case ViewportAction::FlySpeedScroll:
                m_EditorCam.FlyAdjustSpeed(m_ViewInput.GetScrollDelta());
                // Also do the fly update for the frame
                m_EditorCam.FlyUpdate(delta.x, delta.y,
                    m_ViewInput.GetFlyForward(), m_ViewInput.GetFlyRight(), m_ViewInput.GetFlyUp(),
                    dt, m_ViewInput.IsSprinting());
                break;
            case ViewportAction::FocusSelected:
                if (m_Selected) {
                    m_EditorCam.FocusOn(m_Selected->GetTransform().position);
                }
                break;
            case ViewportAction::SnapFront:   m_EditorCam.SnapView(0.0f, 0.0f); PushLog("[Viewport] Front view"); break;
            case ViewportAction::SnapBack:    m_EditorCam.SnapView(180.0f, 0.0f); PushLog("[Viewport] Back view"); break;
            case ViewportAction::SnapRight:   m_EditorCam.SnapView(90.0f, 0.0f); PushLog("[Viewport] Right view"); break;
            case ViewportAction::SnapLeft:    m_EditorCam.SnapView(270.0f, 0.0f); PushLog("[Viewport] Left view"); break;
            case ViewportAction::SnapTop:     m_EditorCam.SnapView(0.0f, -89.0f); PushLog("[Viewport] Top view"); break;
            case ViewportAction::SnapBottom:  m_EditorCam.SnapView(0.0f, 89.0f); PushLog("[Viewport] Bottom view"); break;
            case ViewportAction::ToggleOrtho:
                if (cam->projectionType == ProjectionType::Perspective) {
                    cam->projectionType = ProjectionType::Orthographic;
                    cam->orthoSize = m_EditorCam.GetDistance() * 0.5f;
        ...
        // Smooth-update camera every frame
        ...
        if (m_ViewInput.IsFlying()) {
            ...
            char speedBuf[64];
            std::snprintf(speedBuf, sizeof(speedBuf), "Fly Mode | Speed: %.1f | WASD+QE | Shift=Sprint | Scroll=Speed", m_EditorCam.GetFlySpeed());
...
void EditorUI::AISpawnBlueprintsFrom(const std::vector<AIManager::ObjectBlueprint>& blueprints) {
    ...
    auto parseDirective = [&](const std::string& raw, std::string& physicsRole,
                              std::string& controller, std::string& scriptSource,
        ...
        const std::string snippet = trim(raw);
        if (snippet.empty()) return;

        // Legacy compatibility: plain "physics" flag
        if (toLower(snippet) == "physics") {
            physicsRole = "dynamic";
            return;
        }
        ...
        while (start <= snippet.size()) {
            size_t end = snippet.find(';', start);
            std::string token = trim(snippet.substr(start, end == std::string::npos ? std::string::npos : (end - start)));
            if (!token.empty()) {
                std::string lower = toLower(token);
                auto consumeKV = [&](const std::string& key, std::string& out) {
                    if (lower.rfind(key + ":", 0) == 0) {
                        out = trim(token.substr(key.size() + 1));
                        parsedAny = true;
                        return true;
                    }
                    if (lower.rfind(key + "=", 0) == 0) {
                ...
                if (consumeKV("physics", physicsRole)) {}
                else if (consumeKV("controller", controller)) {}
                else if (consumeKV("script", scriptSource)) {}
                else if (consumeKV("scriptpath", scriptPath)) {}
                else if (consumeKV("script_file", scriptPath)) {}
            }
        ...
        // If no directive syntax is used, treat snippet as raw script source.
        if (!parsedAny) scriptSource = snippet;
    };
    ...
    for (size_t i = 0; i < blueprints.size(); i++) {
        ...
        // ── Directive-based setup (scene-agnostic, editor-driven) ─────────
        std::string physicsRole;
        std::string controller;
        std::string scriptSource;
        std::string scriptPath;
        parseDirective(bp.scriptSnippet, physicsRole, controller, scriptSource, scriptPath);

        std::string role = toLower(trim(physicsRole));
        if (!role.empty()) {
        ...
        std::string ctrl = toLower(trim(controller));
        if (ctrl == "car") {
            obj->AddComponent<CarController3D>();
            PushLog("[AI] Controller 'car' attached to '" + bp.name + "'");
        } else if (ctrl == "force") {
            ...
            PushLog("[AI] Controller 'force' attached to '" + bp.name + "'");
        }

        if (!scriptPath.empty() || !scriptSource.empty()) {
            ...
            if (m_Script) sc->SetEngine(m_Script);
            PushLog("[AI] Script attached to '" + bp.name + "'");
        }
...
void EditorUI::AIGenerate2D() {
    ...
    // Reject very short prompts
    {
        ...
        if (trimmed.size() < 8) {
            m_AIStatusMsg = "Prompt too short — describe a 2D scene (e.g. 'car racing on platforms').";
    ...
    // Use the new 2D generation method
    AIManager::SceneGenResult2D result = m_AI->GenerateScene2DFromPrompt(prompt);

    m_AIProgress = 0.8f;

    if (!result.success) {
        PushLog("[AI 2D] Generation failed: " + result.errorMessage);
        m_AIStatusMsg = "Error: " + (!result.errorMessage.empty() ? result.errorMessage : "No valid objects generated");
        m_AIGenerating = false;
        m_AIProgress = 0.0f;
        return;
    }
    ...
    if (result.objects.empty()) {
        PushLog("[AI 2D] No objects in response.");
        m_AIStatusMsg = "Error: No objects generated.";
        m_AIGenerating = false;
        m_AIProgress = 0.0f;
        return;
    }

    PushLog("[AI 2D] Received " + std::to_string(result.objects.size()) + " objects. Spawning...");
    m_AILast2DSpawnedIDs.clear();
...
// ============================================================================
// 2D AI Object Spawning (ObjectBlueprint2D with Controllers)
// ============================================================================

void EditorUI::AISpawnBlueprints2D(const std::vector<AIManager::ObjectBlueprint2D>& blueprints) {
    auto& scene2d = m_2DViewport.GetScene();
    ...
    for (const auto& bp : blueprints) {
        ...
        // Set transform
        obj->GetTransform().position = Vec3(bp.position.x, bp.position.y, 0.0f);
        obj->GetTransform().scale    = Vec3(bp.scale.x, bp.scale.y, 1.0f);
        obj->GetTransform().SetEulerDeg(0, 0, bp.rotation);

        // Add sprite component
        auto* spr = obj->AddComponent<SpriteComponent>();
        spr->color = bp.color;
        spr->size  = Vec2(bp.width, bp.height);

        // Add physics body if needed
        auto* rb = obj->AddComponent<RigidBody2D>();
        if (bp.physicsType == "static") {
            rb->bodyType = BodyType2D::Static;
        } else if (bp.physicsType == "kinematic") {
        ...
        if (bp.hasCollider) {
            auto* col = obj->AddComponent<Collider2D>();
            if (bp.colliderShape == "circle") {
                col->shape = ColliderShape2D::Circle;
                col->radius = (bp.width + bp.height) * 0.25f;
            } else {
                col->shape = ColliderShape2D::Box;
                col->boxSize = Vec2(bp.width * 0.5f, bp.height * 0.5f);
            }
        }

        // Add controller based on type
        if (bp.controllerType == "car") {
            auto* carCtrl = obj->AddComponent<CarController2D>();
            carCtrl->maxSpeed      = 15.0f;
            carCtrl->acceleration = 20.0f;
            carCtrl->turnSpeed    = 180.0f;
            PushLog("[AI 2D] Attached CarController2D to '" + bp.name + "'");
        } else if (bp.controllerType == "platformer") {
            auto* platformerCtrl = obj->AddComponent<PlatformerController2D>();
            platformerCtrl->moveSpeed = 8.0f;
            platformerCtrl->jumpForce = 14.0f;
            PushLog("[AI 2D] Attached PlatformerController2D to '" + bp.name + "'");
        }
    ...
    // Auto-focus on spawned objects
    if (!blueprints.empty() && !m_AILast2DSpawnedIDs.empty()) {
        Vec2 centre(0, 0);
        int count = 0;
        for (const auto& bp : blueprints) {
            centre = centre + bp.position;
        ...
        if (count > 0) {
            centre = centre * (1.0f / static_cast<f32>(count));
            m_2DViewport.GetCamera().FocusOn(centre);
        }
    }

    // Select the first spawned object
    if (!m_AILast2DSpawnedIDs.empty()) {
        auto* first = scene2d.FindByID(m_AILast2DSpawnedIDs.front());
        if (first) m_2DViewport.SetSelected(first);
    }
}
